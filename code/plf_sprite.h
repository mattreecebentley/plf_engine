...
private:
	struct frame
	{
		plf::colony<SDL_Rect> collision_blocks; // per-frame collision blocks (optional, not required)
		plf::texture *texture;
...
public:
	...
	void get_collision_blocks(const unsigned int frame_number, plf::colony<SDL_Rect> &current_collision_blocks);
