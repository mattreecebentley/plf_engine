...
namespace plf
{
		...
		music(NULL)
	{
		...
		plf_fail_if (SDL_Init(SDL_INIT_VIDEO|SDL_INIT_EVENTS|SDL_INIT_TIMER) < 0, "plf::engine constructor: SDL graphics and subsystems could not initialize! ");

		std::clog << "SDL video, timer and events initialised." << std::endl;
	
		// Initialise SDL_image for use with PNGs/jpegs only:
		plf_fail_if (IMG_Init(IMG_INIT_PNG | IMG_INIT_JPG) == 0, "plf::engine constructor: SDL_image PNG or JPEG support could not initialize! Quitting.");
	
		std::clog << "SDL_img initialised with PNG and JPEG support." << std::endl;
	
	
		// Initialise sound and music:
		plf_fail_if (SDL_Init(SDL_INIT_AUDIO) < 0, "plf::engine constructor: SDL audio could not initialize! ");
		...
		{
			std::clog << "plf::engine constructor: SDL_mixer OpenAudio could not initialize at 44khz, 1024samples. Error: " << Mix_GetError() << std::endl;
	
		 	if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 512) != 0)
		 	{
		  		std::clog << "plf::engine constructor: SDL_mixer OpenAudio could not initialize at 44khz, 512samples. Error: " << Mix_GetError() << std::endl;
	
		 		// Try 48khz instead:
				if (Mix_OpenAudio(48000, MIX_DEFAULT_FORMAT, 2, 1024) != 0)
				{
			 		std::clog << "plf::engine constructor: SDL_mixer OpenAudio could not initialize at 48khz, 1024samples. Error: " << Mix_GetError() << std::endl;

				 	if (Mix_OpenAudio(48000, MIX_DEFAULT_FORMAT, 2, 512) != 0)
				 	{
				  		std::clog << "plf::engine constructor: SDL_mixer OpenAudio could not initialize at 48khz, 512samples. Error: " << Mix_GetError() << std::endl;

					 	// Try 96khz instead:
						if (Mix_OpenAudio(96000, MIX_DEFAULT_FORMAT, 2, 1024) != 0)
						{
					 		std::clog << "plf::engine constructor: SDL_mixer OpenAudio could not initialize at 96khz, 1024samples. Error: " << Mix_GetError() << std::endl;

						 	if (Mix_OpenAudio(96000, MIX_DEFAULT_FORMAT, 2, 512) != 0)
						 	{
						  		std::clog << "plf::engine constructor: SDL_mixer OpenAudio could not initialize at 96khz, 512samples. Error: " << Mix_GetError() << std::endl;

								plf_fail_if (Mix_OpenAudio(96000, MIX_DEFAULT_FORMAT, 2, 2048) != 0, "plf::engine constructor: SDL_mixer OpenAudio could not initialize at 96khz, 2048 samples. Error: " << Mix_GetError());
							}
						}
					}
				}
		...
		// Initialise OGG, MP3 and FLAC SDL_Mixer support:
		const int support_flags = MIX_INIT_OGG|MIX_INIT_FLAC|MIX_INIT_MP3;
		plf_fail_if ((Mix_Init(support_flags) & support_flags) != support_flags, "plf::engine constructor: SDL_mixer OGG/FLAC/MP3 support could not initialize! Quitting.");
	}
