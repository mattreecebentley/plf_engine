...
private:
	struct state
	{
		plf::colony<SDL_Rect> collision_blocks;			 // Collision blocks for state, may be overridden by sprite collision blocks
...
public:
	...
	void get_current_collision_blocks(plf::colony<SDL_Rect> &current_collision_blocks);
	std::string get_id();
	std::string get_type();
