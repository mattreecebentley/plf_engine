...
namespace plf
{
	...
	{
		// Try for hardware acceleration with supplied vsync setting, rendering to textures, fallback to software rendering and/or with inverse vsync setting if unavailable:
		s_renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED|((vsync_mode == VSYNC_ON) ? SDL_RENDERER_PRESENTVSYNC : 0)|SDL_RENDERER_TARGETTEXTURE);
	
		if (s_renderer == NULL)
		{
			std::clog << "plf::renderer constructor: Renderer could not be created, trying without texture rendering." << std::endl;
			s_renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED|((vsync_mode == VSYNC_ON) ? SDL_RENDERER_PRESENTVSYNC : 0));
	
			if (s_renderer == NULL)
			{
				std::clog << "plf::renderer constructor: Renderer could not be created, trying with inverse vsync setting." << std::endl;
				s_renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED|((vsync_mode != VSYNC_ON) ? SDL_RENDERER_PRESENTVSYNC : 0));
	
				if (s_renderer == NULL)
				{
					std::clog << "plf::renderer constructor: Renderer could not be created, trying software." << std::endl;
					s_renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_SOFTWARE|((vsync_mode == VSYNC_ON) ? SDL_RENDERER_PRESENTVSYNC : 0));
	
					if (s_renderer == NULL)
					{
						std::clog << "plf::renderer constructor: Renderer could not be created, trying software with inverse vsync setting." << std::endl;
						s_renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_SOFTWARE|((vsync_mode != VSYNC_ON) ? SDL_RENDERER_PRESENTVSYNC : 0));
						plf_fail_if (s_renderer == NULL, "plf::renderer constructor: Renderer could not be created!");
					}
				}
			}
		}
		...
		plf_fail_if (test_surface == NULL, "plf::renderer Constructor: Could not create RGB test surface! ");
		...
		plf_fail_if (test_texture == NULL, "plf::renderer Constructor: Could not create RGB test texture! ");
		...
		{
			std::clog << "plf::renderer Constructor possible issue: Texture alpha modulation not supported. " << "." << std::endl;
		}
		
		if (SDL_SetTextureColorMod(test_texture, 75, 25, 255) < 0)
		{
			std::clog << "plf::renderer Constructor possible issue: Texture color modulation not supported. " << "." << std::endl;
		}
