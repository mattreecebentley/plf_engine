	...
	renderer(p_renderer)
{
	...
	plf_fail_if (atlas_texture == NULL, "plf::atlas initialisation Error: Unable to create texture of size " << atlas_width << "/" << atlas_height << ". SDL Error: " << SDL_GetError());
...
atlas_node * atlas::add_surface(SDL_Surface *new_surface)
{
	...
	if (new_surface->format->format != renderer->get_surface_pixel_format())
	{
		...
		plf_fail_if (surface == NULL, "plf::atlas add_surface Error: Unable to create surface of size " << width << "/" << height << ". SDL Error: " << SDL_GetError());

		// Convert new_surface to texture pixel format:
		plf_fail_if (SDL_ConvertPixels(width, height, new_surface->format->format, new_surface->pixels, new_surface->pitch, renderer->get_surface_pixel_format(), surface->pixels, surface->pitch) < 0, "plf::atlas add_surface Error: Unable to convert surface of size " << width << "/" << height << ". SDL Error: " << SDL_GetError());
	...
	plf_fail_if (return_value < 0, "plf::atlas add_surface Error: Unable to copy surface of size " << width << "/" << height << " to atlas texture. SDL Error: " << SDL_GetError());
